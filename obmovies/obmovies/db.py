...
def get_db():
    ...
    MONGO_POOLSIZE = current_app.config["MONGO_POOLSIZE"]
    MONGO_TIMEOUTMS = current_app.config["MONGO_TIMEOUTMS"]
    if db is None:
...
def get_movies(filters, page, movies_per_page):
    ...
    offset = page * movies_per_page # total amount of movies in previous pages
    movies = cursor.limit(movies_per_page).skip(offset)
